...
diagnostic reasons. The broad historical bucket is subdivided when the cause is
recognizable without guessing source identity:
...
This is classification, not full recovery. For example,
`project_class_present_method_absent_record_component_accessor` tells you that
SootUp reported a record accessor generated by Java's record semantics, but
CoCoMUT did not emit a normal documented source method for it. That edge keeps
its bytecode `target_uri` and leaves `method_uri` empty.
...
phase_3_call_graph_artifact_exists true when SootUp produced an artifact/result set
...
phase_5_call_edges_serialized      total caller + callee edges emitted in JSONL rows
...
## Low Source-Join Follow-Up

A focused run on four low source-join repositories is documented in:

- `docs/research/CALL_EDGE_LOW_JOIN_INVESTIGATION_2026_06_22.md`

The run confirmed that the largest remaining internal-ish issue is not overload
ambiguity. It is usually bytecode methods reported for project classes that are
not present in the selected source-method set, plus nested bytecode class cases.
Those cases remain bytecode-only unless CoCoMUT can identify one source method
deterministically.
